...
**Speck 0.5.0.1 is out!!**
```xml
<dependency>
    <groupId>io.github.pablogrisafi1975</groupId>
...
public class SimpleExample {
    ...
    public static void main(String[] args) {
        ...
        //curl -w "\nhttp_code:%{http_code}\ncontent_type:%{content_type}" localhost:5678/hello
        get("/hello", (request, response) -> "Hello World!");

        //curl -w "\nhttp_code:%{http_code}\ncontent_type:%{content_type}" localhost:5678/hello-plain
        get("/hello-plain", (request, response) -> {
            response.header("Content-Type", "text/plain");
            return "Hello World!";
        });

        //curl localhost:5678/hello -d this-is-the-body
        post("/hello", (request, response) ->
            "Hello World: " + request.body()
        );

        //curl -w "\nhttp_code:%{http_code}" localhost:5678/private
        get("/private", (request, response) -> {
            response.status(401);
            return "Go Away!!!";
        });

        //curl -w "\nhttp_code:%{http_code}" localhost:5678/users/smith
        get("/users/:name", (request, response) -> "Selected user: " + request.params(":name"));

        //curl -w "\nhttp_code:%{http_code}\ncontent_type:%{content_type}" localhost:5678/news/sports
        get("/news/:section", (request, response) -> {
            response.type("text/xml");
            return "<?xml version=\"1.0\" encoding=\"UTF-8\"?><news>" + request.params("section") + "</news>";
        });

        //curl -w "\nhttp_code:%{http_code}\ncontent_type:%{content_type}" localhost:5678/protected
        get("/protected", (request, response) -> {
            halt(403, "I don't think so!!!");
            return null;
        });

        //curl -w "\nhttp_code:%{http_code}\nredirect_url:%{redirect_url}" localhost:5678/redirect
        get("/redirect", (request, response) -> {
            response.redirect("/news/world");
            return null;
        });

        //curl -w "\nhttp_code:%{http_code}\ncontent_type:%{content_type}" localhost:5678/
